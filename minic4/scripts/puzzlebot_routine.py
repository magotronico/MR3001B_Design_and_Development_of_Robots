...
class PuzzleBotMover:
    def __init__(self):
        # Initialize the ROS node
        rospy.init_node('puzzlebot_routine', anonymous=True)

        # Create a publisher to send Twist messages to the robot
        self.publisher = rospy.Publisher('/puzzlebot_1/base_controller/cmd_vel', Twist, queue_size=10)

        # Create a Twist message instance
        self.twist = Twist()

        self.linear_velocity = 0.28  # Linear speed in m/s (nominal speed)
        self.angular_velocity = 1.54  # Angular speed in rad/s (nominal speed)
    ...
    def move_forward(self):
        ...
        # Calculate the duration to move for 4 times the side length
        duration = 4 * self.side_length / self.linear_velocity  # 4 is a magic number
    ...
    def rotate(self, angle):
        ...
        # Calculate the duration to rotate for 2 times the given angle
        duration = 2 * angle / self.angular_velocity  # 2 is a magic number
        rospy.loginfo("Duration: %s", duration)
...
if __name__ == '__main__':
    try:
        puzzlebot_mover = PuzzleBotMover()
        puzzlebot_mover.start_routine()
    except rospy.ROSInterruptException:
        puzzlebot_mover.stop()
        pass